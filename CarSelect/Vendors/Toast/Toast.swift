...
fileprivate extension UIView {
    ...
    /**
     ...
     @param position The toast's position
     */
    func _makeToastActivity(_ position: ToastPosition = .center, isHideKeyboard: Bool) {
        // sanity
        guard objc_getAssociatedObject(self, &ToastKeys.activityView) == nil else { return }

        let toast = _createToastActivityView(position: position)
        let point = position._centerPoint(forToast: toast, inSuperview: self)
        _makeToastActivity(toast, point: point)
        isUserInteractionEnabled = isHideKeyboard ? false : true
    }
...
extension UIView {
    ...
    /// Popup toast
    /// - Parameter message: the message you wanna show
    /// - Parameter duration: ToastManager.shared.duration by default
    /// - Parameter position: ToastManager.shared.position by default
    /// - Parameter title: empty by default
    /// - Parameter image: empty by default
    /// - Parameter style: ToastManager.shared.style by default
    ...
    /// Popup activity
    /// - Parameter position: ToastPosition.center by default
    public func makeActivity(
        _ position: ToastPosition = .center,
        isHideKeyboard: Bool = true
    ) {
        _makeToastActivity(position, isHideKeyboard: isHideKeyboard)
    }
    ...
    /// Hide toast
    /// - Parameter includeActivity: includeActivity
    /// - Parameter clearQueue: clearQueue
